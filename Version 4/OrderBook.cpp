    ...
    while (getline(input_file, line)) {
        ...
        if (new_order.IsNotRejected()) {
            ...
            if (new_order.order_side == 1) {
                ExecuteOrders(order_books[book_index][1], new_order, output_file);  // Match buy order with sell orders
                InsertOrderToBook(order_books[book_index][0], new_order, 1);
            } else {
                ExecuteOrders(order_books[book_index][0], new_order, output_file);  // Match sell order with buy orders
...
void ExecuteOrders(vector<Order>& opposite_orders, Order& new_order, ofstream& output_file) {
    ...
    auto exact_match = find_if(opposite_orders.begin(), opposite_orders.end(),
